...
class StatusReportService
{
            ...
            $query = $this->ticketRepository->getTicketQuery()
                                            ...
                                            ->where(function($q) use($request){
                                                ...
                                                if(isset($request->status_id)){
                                                    $q->where('tickets.status_id', $request->status_id);
                                                }
                                            });
            $pagination = true;
    ...
    public function dateWiseReportDownloadCsv($request){
        ...
        $reportData = $this->getDateWiseReport($request)->getData();
            ...
            return Excel::download(new Export($data), "Status Report for {$reportData->query_data->start_date} to {$reportData->query_data->end_date}.csv");

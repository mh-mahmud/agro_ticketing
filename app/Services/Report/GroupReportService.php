...
class GroupReportService
{
            ...
            $query = $this->ticketRepository->getTicketQuery()
                                            ...
                                            ->where(function($q) use($request){
                                                ...
                                                if(isset($request->group_id)){
                                                    $q->where('tickets.group_id', $request->group_id);
                                                }
                                            });
            $pagination = true;
    ...
    public function dateWiseReportDownloadCsv($request){
        ...
        $reportData = $this->getDateWiseReport($request)->getData();
            ...
            return Excel::download(new Export($data), "Group Report for {$reportData->query_data->start_date} to {$reportData->query_data->end_date}.csv");

...
class TypeReportService
{
    ...
    public function getDateWiseReport($request)
    {
        ...
        try{
            ...
            DB::enableQueryLog();
            $query = $this->ticketRepository->getReportTicketQuery()
                                            ->whereBetween('tickets.created_at', [$startDate . ' 00:00:00', $endDate . " 23:59:59"])
                                            ->where(function($q) use($request){
                                                // If type_id not given then get all source
                                                if(isset($request->type_id)){
                                                    $q->where('tickets.type_id', $request->type_id)
                                                    ->orwhere('types.parent_id', $request->type_id);
                                                }
                                            });
    ...
    public function dateWiseReportDownloadCsv($request){
        ...
        $reportData = $this->getDateWiseReport($request)->getData();
            ...
            return Excel::download(new Export($data), "Type Report for {$reportData->query_data->start_date} to {$reportData->query_data->end_date}.csv");

...
import {Select, Alert, Spin, Input,Button, Modal, AutoComplete, message} from 'antd';
import {API_URL, CRM_API, ANTD_MESSAGE_MARGIN_TOP} from "../../Config";
...
class TicketForm extends Component {
        ...
        if (this.props.match.params.id != undefined) {
            ...
            this.getTicket(id).then((response) => {
                if (response.data.status == 404) {
                    ...
                    // ticketFormData.customer_name = ticketDataForEdit.customer_name;
                    // ticketFormData.mobile_no = ticketDataForEdit.mobile_no;
                    // ticketFormData.address = ticketDataForEdit.address;
                    // ticketFormData.customer_id = ticketDataForEdit.customer_id;
                    // ticketFormData.customer_type = ticketDataForEdit.customer_type;
                    ...
                    // ticketFormData.account_no = ticketDataForEdit.account_no;
                    // ticketFormData.card_no = ticketDataForEdit.card_no;
                    // ticketFormData.cif_id = ticketDataForEdit.cif_id;

                    if (ticketDataForEdit.type && ticketDataForEdit.type.parent == null) {
                        ...
                        // Its a sub type
                        ticketFormData.type_id = types.defaultValue = ticketDataForEdit.type && ticketDataForEdit.type.parent_id == 0 ? /* Type */ ticketDataForEdit.type.id : /* Sub type */ ticketDataForEdit.type.parent_id;
                        ticketFormData.sub_type_id = ticketDataForEdit.type && ticketDataForEdit.type.parent_id != 0 ? ticketDataForEdit.type.id : 0;
    ...
    async getAndSetContactOptions(query, callback) {
        ...
        async function call() {
            ...
            if(regCustomer && Array.isArray(regCustomer.data.customer_ids) && regCustomer.data.customer_ids.length){
    ...
    validateTicketData(additionalRule = null) {
        let rules = {
            // customer_name: Joi.string().min(3).max(200).label('Title').required().messages({
            //     'string.empty': `Customer Name cannot be empty`
            // }),
            // mobile_no: Joi.string().min(11).max(14).label('Mobile NO').required().messages({
            //     'string.empty': `Mobile cannot be empty`
            // }),
            // customer_type: Joi.string().label('Customer Type').required().messages({
            ...
            contact_id: Joi.number().label('Contact').required().messages({
                'number.empty': `Contact cannot be empty`,
            ...
            priority_id: Joi.number().label('Priorities').required().messages({
                'number.empty': `Priorities cannot be empty`
            }),
    ...
    // async addNew(){
    //     const container = document.querySelector('.ant-select-selector');
    //     if (container) {
    //         // Find the element containing the phone number
    //         const phoneNumberElement = container.querySelector('.ant-select-selection-item i.bi-phone-fill');

    //         if (phoneNumberElement) {
    //             const textContent = phoneNumberElement.parentElement.textContent;
        
    //             const phoneNumberMatch = textContent.match(/\d{11}/);
    ...
    //                     user: {
    //                         ...prevState.user,
    //                         mobileSearch: phoneNumber,
    //                     }
    //                 }));
    //                 this.setState({mobileSearchReadOnly: true});
    //                 let checkCustomerResponse = await this.checkCustomerExist(phoneNumber);
    //                 console.log(checkCustomerResponse)
    //                 if(checkCustomerResponse && checkCustomerResponse.data && checkCustomerResponse.data.data > 0) {
    //                     this.setState({disbaleButton: true});
    //                     console.log('disbaleButton', this.state.disbaleButton)

    //                 }
    //                 let response = await this.getCustomerInfo(phoneNumber);
    //                 if(response && response.data) {
    //                     this.setState((prevState) => ({
    //                         user: {
    //                             ...prevState.user,
    ...
    //                         this.setState({isInputReadOnly: true});
    ...
    //     const cli = parts[parts.length - 1]; 
    //     const phonePattern = /^\d{10}$/; // Assumes a 10-digit format, you can modify this based on your needs

    //     let validPhoneNo = phonePattern.test(cli);
    //     const container = document.querySelector('.ant-select-selector');
    //     if (container) {
    //         // Find the element containing the phone number
    //         const phoneNumberElement = container.querySelector('.ant-select-selection-item i.bi-phone-fill');
    ...
    //     const { mobileSearch, firstName, address } = this.state.user;
    ...
    //     if (!mobileSearch) {
    //         mobileSearchErr = 'Mobile is required.';
    //     }

    //     // Check if customer name is empty
    //     if (!firstName) {
    //         firstNameErr = 'Customer Name is required.';
    //     }

    //     // If either of the fields is empty, set the error messages and exit the function
    //     if (mobileSearchErr || firstNameErr) {
    //         this.setState({ mobileSearchErr, firstNameErr });
    //         return; // Exit the function without making the API call
    //     }
    //     this.setState({submitLoading: true});
    //     let response = await this.getOrRegisterCustomer(this.state.user.mobileSearch);
    ...
    async addNew(){
        if (typeof this.props.match.params.cli != 'undefined') {
            const phoneNumber = this.props.match.params.cli;
            this.setState((prevState) => ({
                user: {
                    ...prevState.user,
                    mobileSearch: phoneNumber,
    ...
    async searchForNonRegisterCustomer() {
        const { mobileSearch, firstName } = this.state.user;
        ...
        // If either of the fields is empty, set the error messages and exit the function
        if (mobileSearchErr || firstNameErr) {
            this.setState({ mobileSearchErr, firstNameErr, submitLoading: false });
            return; // Exit the function without making the API call
        }
        ...
        try {
            const response = await this.getOrRegisterCustomer(this.state.user.mobileSearch);
            ...
            if (response.data.user_info && response.data.user_info.status === 200) {
                ...
                if (Array.isArray(response.data.customer_ids) && response.data.customer_ids.length) {
                    this.setCifOptions(response.data.customer_ids);
                }
                sessionStorage.setItem('user_save', "Data Saved Successfully.");
                
                this.resetModal();
            } else if (response.data.user_info && response.data.user_info.errors !== undefined) {
                this.setState({
                    mobileSearchErr: (new Helper()).arrayToErrorMessage(response.data.user_info.errors),
                    submitLoading: false
                });
                sessionStorage.setItem('user_save', response.data.user_info.errors);
                

            } else {
                // Handle other response statuses or scenarios as needed
                sessionStorage.setItem('user_save', "Unexpected response");
                this.setState({ submitLoading: false });
            }
        } catch (error) {
    ...
    async getOrRegisterCustomer(user){
        // return await (new Api()).call('POST', CRM_API + '?TYPE=STORE_CUSTOMER&CLI=' + user, [], (new Token()).get());
            return await (new Api()).call('GET', CRM_API + '?TYPE=STORE_CUSTOMER&CLI=' + user+'&firstName='+ this.state.user.firstName+'&address=' + this.state.user.address,[], (new Token()).get());

    }

    async checkCustomerExist(phoneNumber){
        return await (new Api()).call('GET', API_URL + '/check-user?mobile_no=' + phoneNumber,[], (new Token()).get());

    }

    async getCustomerInfo(phoneNumber){
            return await (new Api()).call('GET', CRM_API + '?TYPE=GET_CUSTOMER_BY_MOBILE&CLI=' + phoneNumber,[], (new Token()).get());
            ...
            <Modal
                ...
                <div>
                    ...
                    <input type="text" className="form-control" id="ts-d-ticket-acc-no"
                            ...
                            readOnly={this.state.mobileSearchReadOnly}
                            value={this.state.user.mobileSearch}/>
                    {this.state.mobileSearchErr ? <strong className='text-danger'>{ this.state.mobileSearchErr }</strong> : null}
                </div>
                <div>
                    <label htmlFor="ts-d-ticket-acc-no">Customer Name</label>
                    <input
                        type="text"
                        className="form-control"
                        id="ts-d-ticket-acc-no"
                    ...
                    />
                    {this.state.firstNameErr ? <strong className='text-danger'>{ this.state.firstNameErr }</strong> : null}
                </div>
                <div>
                    <label htmlFor="ts-d-ticket-acc-no">Address</label>
                    <TextArea
                        rows={2}
                        placeholder="Address"
                        id="ts-d-address"
            ...
            <form id="ts-d-create-form" className={"p-2 p-md-5"}>
                ...
                <div className="row">
                    ...
                    {/* <div className="form-group col-lg-6">
                        <label htmlFor="ts-d-ticket-customer_name"><i className="bi bi-star-fill"></i>Customer Name</label>
                        <input type="text" 
                            className="form-control" 
                        ...
                        />
                        <small className="text-danger">{this.state.validationError.customer_name}</small>

                    </div> */}
                     {/* <div className="form-group col-lg-6">
                        <label htmlFor="ts-d-ticket-mobile_no"><i className="bi bi-star-fill"></i>Mobile NO</label>
                        <input type="text" 
                            className="form-control" 
                            id="ts-d-mobile_no" 
                        ...
                        />
                        <small className="text-danger">{this.state.validationError.mobile_no}</small>

                    </div> */}
                     {/* <div className="form-group col-lg-6">

                        <label htmlFor="ts-d-ticket-address">Address</label>
                        <TextArea
                            rows={2}
                            placeholder="Address"
                            id="ts-d-address"
                    ...
                    {/* <div className="form-group col-lg-6 d-flex align-items-center"><small><i className="bi bi-star-fill text-danger star-icon"></i></small>Customer Type:
                        ...
                        <small className="text-danger">{this.state.validationError.customer_type}</small>
                    </div> */}
                    <div className="form-group col-lg-6">
    ...
    render() {
        const userSaveMessage = sessionStorage.getItem('user_save');
        if (userSaveMessage) {
            message.success({
              content: userSaveMessage,
              style: {
                marginTop: ANTD_MESSAGE_MARGIN_TOP, // You can define ANTD_MESSAGE_MARGIN_TOP
              },
            });
      
            // Clear the success message from sessionStorage after displaying it
            sessionStorage.removeItem('user_save');
          }
        return (
            <>

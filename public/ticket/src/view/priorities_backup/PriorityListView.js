...
export default class PriorityListView extends React.Component {
    ...
    async getAll(page = null, query = null){
        if (query == null){
            return await (new Api()).call('GET', API_URL + '/priorities?page=' + page, [], (new Token()).get(),this.params);
        } else{
            return await (new Api()).call('GET', API_URL + '/priorities?page=' + page + '&query='+ query, [], (new Token()).get());
        }

    }

    async getAdd(){
        return await (new Api()).call('POST', API_URL + '/priorities', this.state.viewInfo, (new Token()).get());
    }

    async getUpdate(){
        return await (new Api()).call('PUT', API_URL + `/priorities/${this.state.viewInfo.id}`, this.state.viewInfo, (new Token()).get());
    }

    async getDelete(){
        return await (new Api()).call('DELETE', API_URL + `/priorities/${this.state.viewInfo.id}`, [], (new Token()).get());
    }
    ...
    addAction(){
        ...
        this.setState({viewModalTitle:"Add Priority"});
    ...
    viewAction(item){
        this.setState({viewSpinning:true});
        this.setState({viewModalTitle:"Priority Details"});
        this.setState({isViewModalVisible:true});

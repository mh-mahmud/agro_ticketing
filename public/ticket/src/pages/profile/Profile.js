...
class Profile extends React.Component {
    constructor() {
        super();
            ...
            errors              : null,
            errorsChngPwd              : null,
            isChngPwdModalVisible:false,
            submitLoading: false,
            submitLoadingChngPwd: false,
        ...
        this.getUserChngPwdData = this.getUserChngPwdData.bind(this);
        this.showModal = this.showModal.bind(this);
        this.showChngPwdModal = this.showChngPwdModal.bind(this);
        this.handleOk = this.handleOk.bind(this);
        this.handleOkChngPwd = this.handleOkChngPwd.bind(this);
        this.handleCancel = this.handleCancel.bind(this);
        this.handleCancelChngPwd = this.handleCancelChngPwd.bind(this);
        this.updateUser = this.updateUser.bind(this);
        this.updateUserChngPwd = this.updateUserChngPwd.bind(this);
        this.Auth = new Auth();
    ...
    async getUserChngPwdData(pageNumber = 1) {
        const response = await (new Api()).call('GET', API_URL + '/user/showOwnData/' + this.state.user_info.id, [], (new Token()).get());
        if (response) {
            this.setState({ userChngPwdData: response.data.user_info }, ()=>{
                this.setState({isAllStateReady: true});
            });
        }
    }
    ...
    showChngPwdModal() {
        this.setState({ isChngPwdModalVisible: true });
    };
    handleOk() {
	  ...
	  handleOkChngPwd() {
        ...
        this.setState({ isChngPwdModalVisible: false });
    };
    ...
    onChangeHandler = (event, key) => {
	    ...
	    onChangeHandlerChngPwd = (event, key) => {
        const { userChngPwdData } = this.state;
        userChngPwdData[event.target.name] = event.target.value;
        this.setState({ userChngPwdData });
    }
    ...
    resetModal() {
	    ...
	    resetChngPwdModal() {
        this.setState({ isChngPwdModalVisible: false });
        this.setState({ submitLoadingChngPwd: false });
        this.setState({ errorsChngPwd: null });
    }
    ...
    validateUserData() {
	 ...
	 validateUserChngPwdData() {
        let rules = {
            current_password: Joi.string().required().label("Password"),
            password: Joi.string().allow('', null).min(6).label('Password'),
    ...
    async updateUserChngPwd() {
        this.setState({ submitLoadingChngPwd: true });
        this.setState({ errorsChngPwd: null });

        let schema = this.validateUserChngPwdData();
        ...
        try {
            ...
            //console.log(this.state.user_info);
            let userChngPwdData = {};
            userChngPwdData.current_password = this.state.userChngPwdData.current_password;
            userChngPwdData.password = this.state.userChngPwdData.password;
            userChngPwdData.confirm_password = this.state.userChngPwdData.confirm_password;
            //user_info.roles = this.state.user_info.roles;

            const data = await schema.validateAsync(userChngPwdData);
            var formData = this.makeformChngPwdData(data);
            let response = await (new Api()).call('POST', API_URL + '/user/change-password/' + this.state.user_info.id, formData, (new Token()).get());

            if (response.data.status_code == 200) {
                this.resetChngPwdModal();
                this.getUserChngPwdData();
                /**
                 * @Developed by @mominriyadh  on 1/30/2022
                 * @URL https://ant.design/components/notification/*
                ...
                this.setState({ submitLoadingChngPwd: false });
                this.setState({ errorsChngPwd: ((new Helper).arrayToErrorMessage(response.data.errors)) });
            }

        }catch(err){
            const { validationErrorChngPwd } = this.state;
            this.setState({ submitLoadingChngPwd: false });
            validationErrorChngPwd[err.details[0].context.key] = err.details[0].message;
            this.setState({ validationErrorChngPwd: validationErrorChngPwd });
            validationErrorChngPwd[err.details[0].context.key] = '';
        }
        ...
        return <>
            ...
            */}
                            ...
                            <div className="ts-d-profile-r-contents mt-5">
                                <div className="ts-d-profile-meta">
                                    <div className="ts-d-profile-designation mb-4">
                                        ...
                                        <div className="ts-d-profile-edit" onClick={this.showChngPwdModal}>Change Password</div>
                                    </div>
                                    ...
                                    <div className="ts-d-profile-details">
                                        ...
                                        <p>
                                            <span className="ts-d-personal">
                                                <i className="bi bi-people-fill" /> Group: { this.state.userData.groups !== undefined && this.state.userData.groups.length != 0 ? this.state.userData.groups.map((item, index) => (<span key={item.id}>{item.name}{index < this.state.userData.groups.length - 1 && ','}</span>)) : 'N/A'}</span>
                                        </p>
                                        <p>
            ...
            <Modal
                ...
                zIndex="1050"
                centered
                title="Edit Change Password" visible={this.state.isChngPwdModalVisible} onOk={this.handleOkChngPwd} 
                footer={[
                    <Button key="back" onClick={this.handleCancelChngPwd}>
                        Close
                    </Button>,
                    <Button
                        className={this.state.actionType === "view" ? "d-none" : ""}
                        loading={this.state.submitLoadingChngPwd}
                        key="Update"
                        type="primary"
                        onClick={this.updateUserChngPwd}

                    >
                        Submit
                    </Button>
                ]} 
                onCancel={this.handleCancelChngPwd}>
                {
                    this.state.errorsChngPwd ?
                        <>
                            <Alert
                                message="Something Wrong !"
                                description={this.state.errorsChngPwd}
                                type="error"
                                closable
                            />
                        </>
                ...
                <form className="row g-3 needs-validation" encType="multipart/form-data">
                    ...
                    <div className="col-md-12 mb-3">
                        ...
                        <input type="password" className="form-control" name="current_password"
                            value={this.state.userChngPwdData.current_password} id="current_password" placeholder="Current Password"
                            onChange={this.onChangeHandlerChngPwd} />
                        <div className="text-danger">{this.state.validationErrorChngPwd.current_password}</div>
                    </div>
                    <div className="col-md-12 mb-3">
                        <label className="form-label">New Password <span className="text-danger">*</span></label>
                        <input type="password" className="form-control" name="password" id="password" placeholder="New Password"
                            onChange={this.onChangeHandlerChngPwd} />
                        <div className="text-danger">{this.state.validationErrorChngPwd.password}</div>
                    </div>
                    <div className="col-md-12 mb-3">
                        ...
                        <input type="password" className="form-control" name="confirm_password" id="confirm_password" placeholder="Confirm Password"
                            onChange={this.onChangeHandlerChngPwd} />
                        <div className="text-danger">{this.state.validationErrorChngPwd.confirm_password}</div>
                    </div>
                </form>
            </Modal>
        </>

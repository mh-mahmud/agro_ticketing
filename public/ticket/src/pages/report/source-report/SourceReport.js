...
class SourceReport extends Component {
    ...
    async getReportData(query = '') {
        return await (new Api()).call('GET', API_URL + '/sourceReportDateWise?page=' + this.state.pageNumber + query, [], (new Token()).get());
    }
    ...
    makeTableData() {
        ...
        let query = '';
        query += this.state.searchFormData.source_id ? `&source_id=${this.state.searchFormData.source_id}` : '';
        ...
        this.getReportData(query).then((response) => {
            ...
            switch(response.data.status) {
                ...
                break;
                case 400:// Error
                    this.state.errorMsg = (new Helper).arrayToErrorMessage(response.data.errors);
                break;
            }
    ...
    dateOnchange(dateString, target){
        ...
        this.setState({searchFormData});
        console.log(dateString, searchFormData);
    }

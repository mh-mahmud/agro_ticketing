...
class GroupReport extends Component {
    ...
    async getReportData(query = '') {
        return await (new Api()).call('GET', API_URL + '/groupReportDateWise?page=' + this.state.pageNumber + query, [], (new Token()).get());
    }
    ...
    makeTableData() {
        ...
        let query = '';
        query += this.state.searchFormData.group_id ? `&group_id=${this.state.searchFormData.group_id}` : '';
        ...
        return (
            <Layout>
                ...
                <Form onChangeHandler       ={this.onChangeHandler}
                      groupChangeHandler     ={this.groupChangeHandler}
...
export default withRouter(GroupReport);

...
class TypeReport extends Component {
    ...
    async getReportData(query = '') {
        return await (new Api()).call('GET', API_URL + '/typeReportDateWise?page=' + this.state.pageNumber + query, [], (new Token()).get());
    }
    ...
    makeTableData() {
        ...
        let query = '';
        query += this.state.searchFormData.type_id ? `&type_id=${this.state.searchFormData.type_id}` : '';
...
export default withRouter(TypeReport);
